...
- [x] Literal server-owned `modernArt` plus compatible registration of all six
      completed atlases selects one atomic modern owner across shell, fighter
      selection, current-map gameplay, HUD/minimap/menu, and Results. Any
      missing atlas restores the complete legacy/procedural owner.
- [x] Current 960x576 map JSON projects deterministically to the biome atlas
      without changing bounds, collision, destruction, chunks, navigation,
      spawns, objectives, pickups, landmarks, names, or minimap truth. Animated
      gates and scavenger caches retain explicit compatible legacy fallbacks.
- [x] Muzzle, scenery impact, authoritative confirmed-player impact, exact-
      radius explosion, healing, armor, all fighter releases, and elimination
      have one modern graphical owner. Audio, lighting, camera, decals, scorch,
      timing, direction, 32/16 budgets, and server event truth remain unchanged.
- [x] Fighter selection and Results reuse all six roster identities, Jack's
      carried axe, Rook's aligned helmet, modern UI state/focus hierarchy, and
      responsive desktop/mobile-landscape safe areas. Bat/punch and every
      incompatible carried-object state retain legacy presentation.
- [x] SMG, sniper, launcher, container, rarity, and zone mechanics remain
      dormant. No live caller, gameplay/system authority, shared/server/wire,
      map schema, capability default/exposure, production, or deployment change
      occurs, and no legacy asset is removed or stops loading.
- [x] Deterministic cutover, availability, fallback, lifecycle, map projection,
      event ownership, quality, and recovery tests; all asset validators; full
      unit/type/lint/build/format/diff/provenance gates; targeted modern
      Chromium/mobile/Firefox evidence; and the complete default-false three-
      project browser matrix pass under RFG-003.
...
- [x] Deterministic strict `standard-40x24` documents preserve both public
      names and biome identities while supplying readable primary/flank routes,
      defensible escapable landmarks, four quadrant spawns, three legal KOTH
      anchors, the Core Run center, reachable pickup economy, one walkable
      component, destructible cover, gates/barrels, complete minimap metadata,
      and honest asymmetric reviews.
- [x] The additive shared resolver returns either successor only for literal
      `largeWorlds: true`; absent/false paths return the exact legacy objects
      and the other four arenas remain unchanged. Registry order, public/wire
      names, persistence keys, and default matchmaking remain compatible.
- [x] Every authoritative server launch/rematch path resolves from the server
      capability. The client consumes the normalized handshake choice without
      viewport/name/config/art inference or a second selection surface.
- [x] Strict/stable validation, focused positive/negative and cross-package
      tests, full unit/static/build/format/diff/legacy-byte gates, the complete
      default-false and enabled three-project browser matrices, and targeted
      Chromium/Firefox/mobile arena evidence pass under RFG-003. Production,
      capability exposure, 20Hz simulation, and Batch 36+ work remain unchanged.
...
- [x] Checkpoint Zero declares dense horizontal/vertical reinforced-barricade
      routes, paired inspection chokepoints, control posts, gates, and props;
      Rusted Refinery declares a red-roof central power vault, open north/south
      approaches, side gates, pipes, tanks, and industrial processing lanes.
- [x] Each strict version-1 `standard-40x24` document owns four safe spawns,
      reachable pickup economy, three KOTH anchors, centered Core Run,
      destructible cover, existing explosive barrels, legal shootable-gate
      shortcuts, complete minimap metadata, one connected walkable component,
      and an explicit asymmetry review.
- [x] Literal server-owned variant resolution selects all six successors only
      with `largeWorlds: true`; capability-off and old-server paths restore the
      byte-identical legacy documents. The first four successors, public map
      registry/names/wire payloads, and all six legacy JSON files remain unchanged.
- [x] Strict/stable validation, focused and full unit/static/build/format/diff/
      legacy-byte gates, complete default-false and enabled browser matrices,
      and targeted Chromium/Firefox/mobile evidence pass under RFG-003. No
      deployment, capability exposure, rebalance, or Batch 38 work occurred.
...
Selected verification tier (2026-07-17, before any Batch 39 change): complete milestone matrix.
...
**Verification tier selected before implementation (2026-07-18):** shared,
server-simulation, additive-network, and capability-owned client interaction
tier, escalated because the existing reload input and weapon/pickup choke points
must remain byte/behavior compatible for every standard format. Required
deterministic evidence covers fist spawn, exactly one equipped gun, loaded ammo,
universal reserve, ordinary reload from that reserve, dry-gun discard to fists,
unarmed auto-equip, armed no-auto-swap, reload-context intentional swap, the old
gun's exact rarity/loaded-ammo drop, retained player reserve, stable contested
collection, N-player drop state, disconnect/death cleanup boundaries, and
server-authored nearby comparison state. Standard rifle/special ammo, ordinary
pickups, reload, Weapon Roulette, Gun Game, One in the Chamber, Fists Only,
death drops, stats, persistence, input, snapshots, rematches, and fallback must
remain unchanged; optional additions must disappear from standard JSON and
old-server omission must clear client state. Run focused shared/server/client
tests, the complete unit matrix, typecheck, lint, all builds, allowlist/repository
format and diff checks, targeted pointer/keyboard/gamepad/touch browser evidence,
and the authoritative 20 Hz/standard-snapshot baseline. Escalate browser scope
only if the contextual action crosses an established input or scene foundation.

- [x] Battle Royale entrants spawn with fists and one empty server-owned slot.
      Exactly one coherent gun may be equipped; loaded ammo belongs to that gun
      while one bounded universal reserve remains with the fighter.
- [x] An unarmed fighter automatically equips the nearest legal drop. An armed
      fighter receives only a server-authored comparison candidate and swaps
      only on contextual reload; stable player-ID and drop-ID ordering resolves
      simultaneous contests without client collection authority.
- [x] Reload transfers universal reserve into the held magazine. Intentional
      swap drops the old gun with its exact instance, rarity, and loaded ammo
      while retaining reserve; firing the final loaded round discards the empty
      gun to fists. Death and departure clear held inventory without inventing
      Batch 44 elimination loot.
- [x] Inventory and dropped guns are additive optional snapshot state. New
      clients normalize complete coherent state, clear old-server omissions,
      fail malformed arrays closed, render server-owned drops/comparisons, and
      expose keyboard, standard-gamepad, and Battle Royale-only touch reload.
- [x] Standard pickups, rifle/special ammo, reloads, death drops, modes,
      mutators, stats/persistence, and exact 2,481/3,762-byte snapshots remain
      unchanged. The complete 149-file/1,696-test matrix, typecheck, lint, all
      builds, focused three-project browser evidence, formatting/diff checks,
      and authoritative 20 Hz baseline passed.
- [x] Batch 44 containers/loot and all later arena, zone, bot, spectating,
      records, hardening, release, capability exposure, deployment, restart,
      and live-smoke work did not begin.
...
**Verification tier selected before rollout mutation (2026-07-18):** complete
production-rollout matrix. Batch 52 crosses server/client versioning,
capability exposure, persistence, restart, every player-facing journey, and the
live rollback boundary, so it requires the complete unit/static/build and
three-project browser gates plus direct production version, health,
configuration, persistence, capability, schedule, desktop/mobile, standard,
and Battle Royale evidence. Batch 51's release packet remains prerequisite
evidence but does not replace fresh rollout checks.

Roll out in public-release order. First deploy the server candidate with every
capability false and verify the still-live old client. Then deploy the
capability-aware client and verify the complete false-flag fallback. Enable
`newShell`, `schedules`, `largeWorlds`, and `modernArt` together while retaining
`battleRoyale=false`; smoke Reforged Arena and standard rollback paths. Only
after that cohort is healthy may `battleRoyale` become true for the Battle
Royale smoke. A Battle Royale-only issue rolls back that flag first; a broader
issue sets all five server flags false before any client rollback.
...
- [x] Production persistence is backed up and validates before restart; deploy
      steps do not delete or reset standard, Daily, or Battle Royale records.
- [x] The server candidate deploys first with all five flags false, restarts
      under the established PM2 owner, and passes health while the prior client
      retains its complete legacy journey.
- [x] The capability-aware client deploys next and passes production health,
      cache/version, false-flag, old-peer/fallback, and recovery checks before
...
### Batch 33 — 2026-07-17 — Full-journey visual cutover

**Shipped:** Added one atomic scene-selection contract for the literal
server-owned `modernArt` journey. Boot-loaded availability for all six completed
production atlases is now required before the modern owner is selected; one
missing or incompatible atlas restores the complete legacy owner. The modern
owner cuts over the Reforged shell, matchmaking and party presentation,
fighter selection, current small-world map projection, every established live
fighter and carried object, pickups, HUD/minimap, combat feedback,
death/respawn, Results, rematch, and recovery without changing authoritative
truth. Current map JSON is projected into the four biome families while shared
collision and destruction remain untouched. Muzzle, scenery impact,
confirmed-player impact, explosion, healing, armor, fighter release, and
elimination each have one modern graphical owner; bat, punch, incompatible
carried objects, gates, and caches retain explicit legacy fallback ownership.
Rarity and zone art remain dormant. No completed atlas, source sheet, manifest,
runtime metadata, or provenance byte changed.

**Verification:** Selected the cross-cutting interaction/visual tier and
escalated to the complete unit, asset, build, and three-project browser matrices
because the change crosses Boot, navigation, gameplay, Results, and recovery.
All 30 asset validators passed deterministic byte, metadata, grid, runtime-
safety, and provenance checks. The full Vitest matrix passed all 141 files and
1,609 tests, including nine cutover-selection/event-ownership tests, two
all-roster preview tests, and six live-map projection tests. Typecheck, lint,
the affected client build, full production build, repository formatting, and
`git diff --check` passed; Vite's established chunk advisory remains. The
default-false browser inventory covered all 252 Chromium, staged Firefox, and
mobile-landscape cases with 144 passes and 107 intentional capability/project
skips; its only initial failure was a pre-canvas Chromium startup timeout, and
that exact case passed immediately on a fresh isolated server. The enabled
shell matrix passed 33 with three expected capability-off skips, while the
targeted gameplay cutover matrix passed 18 with nine project-role skips.
Desktop Chromium supplied live compositor and resized mobile-width visual
evidence; Firefox/mobile object and input assertions plus direct renderer
pixels supplied the paired staged evidence required by RFG-003. Visual review
confirmed one coherent six-atlas reading order, quiet ground, collision-class
...
registered language-icon frame. The first default-false matrix exposed that new
map diagnostics had extended an exact legacy inspection surface; the diagnostics
were moved to a separate getter and all affected three-project cases passed.
One broad capability invocation incorrectly routed legacy shell cases through
the gated shell and was discarded in favor of the contract-correct matrices.
Repository-wide Prettier exposed unrelated historical formatting drift; only
the explicit Batch 33 allowlist is retained. Managed Windows node-module access
required approved repository-local execution. No pixel generation or import
...
### Batch 35 - 2026-07-17 - Wasteland Outpost and Overgrown Suburb

**Shipped:** Hand-authored deterministic version-1 `standard-40x24`, 40x24,
48px-tile successors for Wasteland Outpost and Overgrown Suburb. Both preserve
their public names and biome identities while declaring primary routes,
flanks, defensible but escapable landmarks, three legal KOTH anchors, the
centered Core Run anchor, four quadrant spawns, reachable pickup economies,
destructible cover, shootable-gate shortcuts, existing explosive-barrel
hazards, complete minimap projection metadata, one connected walkable
component, and explicit asymmetry reviews.

The shared registry now has one private strict-validated successor table and a
literal `{ largeWorlds: true }` variant resolver. Every authoritative server
launch path resolves through the server capability snapshot, and `GameScene`
consumes the normalized handshake choice. Capability-off, absent/malformed
old-server, and recovery paths retain the legacy documents; the public
`MAP_REGISTRY`, public map names, wire payloads, all six legacy JSON bytes, and
the other four current arenas are unchanged. No successor enters default
matchmaking or production, and no map-specific gameplay or client-owned
selection surface was added.

**Verification:** Selected the authored-layout/runtime-loading cross-authority
tier. Both successors passed strict validation and stable-order CLI output;
the map CLI suite passed 3 tests. Focused positive/negative authoring,
registry/selection, connectivity, objectives, spawn/pickup, collision/
destruction, gate/hazard, minimap, dynamic-world, and old-schema coverage
passed 7 files and 166 tests. The full Vitest matrix passed 142 files and
1,623 tests. Typecheck, lint, affected shared/server/client builds, and the
full production build passed; Vite's established chunk advisory remains.
...
166 cases with 89 expected skips in 29.9 minutes. Targeted Chromium,
Firefox, and mobile-landscape arena evidence covered both successors, all
camera edges and spawn quadrants, routes/landmarks, destruction, gates,
barrels, KOTH/Core Run anchors, HUD/minimap, server-owned choice,
capability-off/old-server legacy restoration, Results/rematch, and recovery.
Chromium supplied live/compositor pixels; Firefox/mobile supplied staged
object/input and direct-renderer evidence under RFG-003.

Repository-wide Prettier still reports the established 100 unrelated
historical files; the Batch 35 allowlist is formatted. `git diff --check`,
intended-diff review, stable-order review, and legacy-byte review passed.
...
**Deviations:** No product-scope deviation. The complete enabled browser matrix
exposed two validation-harness assumptions: the rumble-lead bound check assumed
a camera at legacy-world origin, and one three-client rally test had only a
60-second budget despite taking 43.7 seconds in isolation. The assertions now
use the active camera world view, and the orchestration-only rally budget is 90
seconds with a 20-second phase poll. The unchanged rally journey then passed in
37.3 seconds inside the final full matrix. Managed Windows node-module access
required approved repository-local execution. The Codex patch wrapper later
hit a Windows path/access fault, so its narrow mechanical fallback used Git's
patch applicator with exact diff review.
...
### Batch 37 - 2026-07-17 - Checkpoint Zero and Rusted Refinery

**Shipped:** Hand-authored deterministic version-1 `standard-40x24`, 40x24,
48px-tile successors for Checkpoint Zero and Rusted Refinery. Checkpoint Zero
declares barricaded control lanes, inspection chokepoints, paired gates and
props, and dense horizontal/vertical low-cover routes. Rusted Refinery declares
a central power vault and red-roof landmark, open north/south approaches, side
gates, pipes, tanks, and industrial processing lanes. Each document owns
readable primary routes and flanks, defensible but escapable landmarks, three
legal KOTH anchors, centered Core Run, four quadrant spawns, reachable pickup
economy, destructible cover, shootable-gate shortcuts, existing explosive
barrels, complete minimap metadata, one connected walkable component, and an
explicit asymmetry review.

The private successor table now completes all six arenas while the literal
`{ largeWorlds: true }` resolver, authoritative server launch paths, and
normalized client handshake consumption remain unchanged. Capability-off,
absent/malformed old-server, Results/rematch, and recovery paths retain the
legacy documents. The public `MAP_REGISTRY`, public names, wire payloads, all
six legacy JSON bytes, and all four Batch 35-36 successor bytes are unchanged.
No successor enters default matchmaking or production, and no map-specific
gameplay or client-owned selection surface was added.

**Verification:** Selected the authored-layout/runtime-loading cross-authority
tier. All six successors passed strict validation and stable-order CLI output;
all twelve current documents passed compatible loading and the map CLI suite
passed 3 tests. Focused positive/negative authoring, registry/selection,
connectivity, objectives, spawn/pickup, collision/destruction, gate/hazard,
minimap, dynamic-world, and old-schema coverage passed 7 files and 111 tests.
The full Vitest matrix passed 142 files and 1,625 tests. Typecheck, lint,
affected shared/server/client builds, and the full production build passed;
Vite's established chunk advisory remains.
...
166 cases with 89 expected skips: Chromium 63/22, Firefox 51/34, and mobile
landscape 52/33. Targeted Chromium live/compositor and Firefox/mobile staged
object/input plus direct-renderer evidence covered all six successors, both new
arena identities, camera edges, spawn quadrants, routes/landmarks, destruction,
gates, barrels, KOTH/Core Run anchors, HUD/minimap, server-owned choice,
capability-off/old-server legacy restoration, Results/rematch, and recovery
under RFG-003. Direct inspection of the Checkpoint Zero and Rusted Refinery
desktop/mobile captures confirmed readable identity, objective cues, minimap
geometry, and unobstructed HUD/touch surfaces.

Repository-wide Prettier reports 109 unrelated historical/environment files;
the Batch 37 allowlist is formatted. `git diff --check`, intended-diff review,
strict/stable-order review, and legacy/Batch-35-36-successor byte review passed.
...
**Deviations:** No product-scope deviation. The first default-port browser run
reached an unrelated app already occupying port 5173, so the final matrices ran
on isolated server/client ports. Two early enabled launches over-advertised
unrelated `newShell`/`schedules` and then `modernArt` capabilities; their
expected legacy-path mismatches were diagnosed with narrow evidence, stopped,
and replaced by the established literal-`largeWorlds` matrix, which passed in
full. Managed Windows node-module access required approved repository-local
execution.
...
### Batch 43 - 2026-07-18 - Single-slot inventory

**Scope and tier:** Before implementation, the shared/server-simulation,
additive-network, and capability-owned client-interaction tier was selected and
escalated because Batch 43 crosses the established weapon, pickup, reload, HUD,
and all four input boundaries. The batch added only fists, one equipped gun,
loaded/universal ammo, contextual collection/reload/swap, dry discard, and
authoritative dropped-gun state. No container, loot source, elimination pile,
arena, zone, bot AI, spectating, record, hardening, exposure, or release work
entered the allowlist.

**Authority and compatibility:** The server owns the inventory registry,
nearest candidate, stable contested collection order, swap, reload transfer,
ammo consumption, dry discard, and elimination/departure cleanup. A swap drops
the exact former instance and loaded ammo while universal reserve stays with the
fighter. Clients normalize coherent optional state, fail malformed projections
closed, render ground guns/comparisons, and send only the existing reload input
through keyboard, standard gamepad, or a Battle Royale-only touch action.
Standard formats omit both new fields and preserve their weapon/pickup/reload,
death-drop, mode/mutator, stats/persistence, rematch, and input contracts.

**Verification:** Focused shared/server/client evidence passed 72 tests, and the
full matrix passed 149 files and 1,696 tests. The complete matchmaking manager
matrix additionally passed 95 tests for additive Battle Royale fields and
standard omission. Typecheck, ESLint, all package builds, the Batch 43 allowlist
formatting, and `git diff --check` passed. Repository-wide Prettier still reports
96 inherited files, none on the Batch 43 allowlist. The focused projection and
input journey passed desktop Chromium pointer/keyboard, desktop Firefox standard
gamepad, and mobile-landscape touch; Chromium produced the trusted compositor
artifact under RFG-003. The server baseline retained configured/rolling 20 Hz
and the 50 ms budget; synthetic four-player mean/p95/p99/max work was
0.018/0.031/0.123/0.867 ms, while standard active snapshot bytes remained
exactly 2,481/3,762.

**Bug ledger:** RFG-005 records a pre-existing Batch 40 lifecycle gap exposed by
the new launcher/inventory evidence: an uncredited self-explosion in a
three-plus-fighter Battle Royale marked the victim dead without recording the
elimination. The authoritative explosion branch now records that combat event,
clears inventory, and still awards no kill; its deterministic three-fighter
wall-impact reproduction passes. No open defect remains from Batch 43.
...
### Batch 52 - 2026-07-19 - Owner-authorized production canary and RFG-008 correction

**Verification tier:** Complete production-rollout matrix, selected before the
first rollout mutation because this batch crosses server/client versions,
capability exposure, persistence, restart, standard and Battle Royale journeys,
and the live rollback boundary. Human acceptance is deliberately deferred to
the latest safe point after automated and agent-driven production evidence; it
is not waived.

**Initial rollout and rollback:** Production persistence was backed up before
restart. Batch 51 server support was deployed first with all five flags false,
then the capability-aware client passed live false-capability fallback. Reforged
Arena was enabled coherently with `battleRoyale=false` and remained healthy.
The first one-human/seven-bot live Battle Royale smoke reached Results with only
two rows, proving RFG-008. Battle Royale alone was immediately set false and
...
standard matches retain the existing actor-state guard. Three- and eight-entrant
regressions reproduce the real callback order and require complete coherent
placements. The client projects an explicit `BATTLE ROYALE` / one-life briefing
without exposing the internal deathmatch adapter, while every standard briefing
retains the exact shared configuration object. The complete unit inventory
passed 161 files and 1,767 tests; typecheck, ESLint, all production builds, all
13 compatible map documents, four map CLI tests, all six asset manifests, 30
asset tests, the 624-product standard balance matrix, and server baseline passed.

The repeated eight-entrant performance evidence measured 0.199/0.355/0.559/
7.610 ms mean/p95/p99/max synthetic tick work, 13,941/17,876-byte
representative/stressed snapshots, 357,520 bytes/s per client, 2,860,160
bytes/s across eight recipients, one encoding for eight deliveries, 437,736
bytes settled heap growth, and zero active matches after cleanup.

**Browser gate:** The complete capability-off inventory passed 153 tests with
126 intentional skips. The complete coherent-enabled inventory passed 203 tests
with 75 intentional skips and one already documented long-run Chromium
synthetic reload-key miss after its inventory, loot, comparison, and pointer
assertions had passed. The same journey passed in Firefox and mobile within the
...
## Batch 33 input prompt (historical)
...
docs/REFORGED_BASELINE.md, docs/REFORGED_CAPABILITIES.md,
docs/REFORGED_STYLE_BIBLE.md, docs/REFORGED_ASSET_PIPELINE.md, and
docs/reforged/style-bible/PROVENANCE.md before implementation. Batch 33 — Full-
journey visual cutover is complete. Implement Batch 34 — Map authoring contract
exactly as specified and do not begin Batch 35 — Wasteland Outpost and
Overgrown Suburb.

Extend the shared map schema, validation, authoring/tooling, deterministic test
fixtures, and client/server-compatible loading contracts needed for authored
40x24 standard arenas. Define explicit, testable contracts for regions,
landmarks, minimap projection metadata, connectivity, objectives, spawn safety,
pickup placement, shootable gates, authored hazards, and symmetry/asymmetry
review. Keep map data declarative, deterministic, N-player-safe, and consumable
by the existing authoritative server, shared collision/destruction utilities,
dynamic renderer, camera, HUD, and minimap without client inference.

Batch 34 owns contracts and tools only. Do not author or replace the six current
maps, change their names or exact 960x576 `(0, 0)` behavior, enable larger maps
in matchmaking, alter world/camera/input/physics/combat/navigation/bot/mode
balance, or begin Batch 35 map-layout work. Do not add tactical-map gameplay,
Battle Royale systems, rarity/loot/containers, new hazards, spectating,
capability exposure, production rollout, or deployment. Batch 35 owns the first
two 40x24 successor layouts; Batch 38 owns mode/bot rebalance; Batch 39 owns the
...
and recovery behavior byte-for-byte unless a proven contract-only compatibility
change is required. All capabilities remain strict server-owned opt-ins and
default false. Current authoritative simulation remains 20Hz.

Make validation failures stable and actionable. Cover dimensions and bounds,
tile/collision row shape, region coverage and overlap rules, landmark identity
and footprint, objective and pickup reachability, spawn separation and safe
egress, gate/hazard metadata, minimap projection, connectivity, symmetry review,
and old-schema/current-map compatibility with deterministic positive and
negative fixtures. Do not encode art policy, gameplay balance, or client-only
state into the shared authoring contract.

Batch 33 is complete and pushed on main as `feat(art): cut over Reforged visual
journey`. Literal server-owned `modernArt` now selects one atomic modern owner
only when the 48-frame UI, 288-frame fighter I, 404-frame fighter II, 158-frame
weapon/pickup, 80-frame biome, and 96-frame feedback atlases are all compatible.
Current maps project to biome art without changing map/collision/destruction
truth; modern live feedback has no legacy graphical doubles; bat/punch,
animated gate/cache, incompatible carried-object, capability-off/old-server,
missing-atlas, and recovery fallbacks remain registered. Future weapon,
container, rarity, and zone mechanics remain dormant. No atlas bytes, gameplay,
shared/server/wire authority, capability default/exposure, production, or
deployment changed.

Choose and document the Batch 34 shared-contract/tooling verification tier. Run
focused schema/validator/tool tests, the full unit matrix because shared map
contracts cross client/server authority, typecheck, lint, affected and full
production builds, repository formatting, `git diff --check`, intended-diff
review, and risk-based browser regression for current maps, collision/
destruction, viewport/camera/HUD/minimap, capability-off/old-server, Results/
rematch, and recovery. Escalate to the complete three-project browser matrix if
the implementation changes runtime loading or any cross-cutting map consumer.
Update roadmap acceptance/status, architecture/baseline/capability docs, bug
ledger only with proven evidence, and Session Log. Run the complete end-of-
batch ritual, commit and push directly to main, verify a clean worktree with
HEAD exactly matching origin/main, and skip deployment.
...
## Batch 35 input prompt (historical)
...
docs/REFORGED_MAP_AUTHORING.md, docs/REFORGED_STYLE_BIBLE.md,
docs/REFORGED_ASSET_PIPELINE.md, and
docs/reforged/style-bible/PROVENANCE.md before implementation. Batch 35 -
Wasteland Outpost and Overgrown Suburb is complete. Implement Batch 36 -
Scrapyard and Collapsed Overpass exactly as specified and do not begin Batch
37 - Checkpoint and Rusted Warehouse.

Hand-author deterministic 40x24, 48px-tile successor documents for Scrapyard
and Collapsed Overpass against the complete version-1 `standard-40x24`
contract. Preserve their exact public names and recognizable biome identities:
Scrapyard must read as lanes through compressed cars, containers, salvage
stacks, and fenced processing yards; Collapsed Overpass must read as broken
road decks, underpass channels, stranded vehicles, and divided concrete
approaches. Give each readable primary routes, flanks, defensible but escapable
...
Batch 36 owns only these two successor layouts and the minimal additive entries
in the existing Batch 35 variant resolver. Preserve the completed 40x24
Wasteland Outpost and Overgrown Suburb successors byte-for-byte unless a proven
shared-contract correction is required. Capability-off and old-server paths
must keep the byte-identical 20x12 legacy Scrapyard and Collapsed Overpass, all
six legacy JSON files remain unchanged, Checkpoint and Rusted Warehouse remain
legacy-only, and no 40x24 arena may enter default matchmaking or production.
The authoritative server and capability handshake must continue choosing the
map variant; the client may consume that server-owned choice but must not infer
it from viewport size, map name, local configuration, or art availability.
Keep public map names and wire payloads compatible; do not create a second
client-owned map-selection surface.

Do not tune world/camera/input/physics/combat/navigation/bot/mode balance,
author the Batch 37 arenas, or begin Batch 38 rebalance. Do not add tactical-
map gameplay, Battle Royale systems, rarity/loot/containers, new hazard types,
spectating, capability exposure, production rollout, or deployment. Batch 38
owns cross-arena mode/bot rebalance; Batch 39 owns the Reforged Arena release
gate, production review, and any capability exposure.
...
and recovery behavior byte-for-byte. Preserve Batch 34 stable validation codes,
strict/compatible loading profiles, fail-closed result, stable-order tooling,
and all old-schema fixtures. Preserve Batch 35's literal server-owned variant
selection, legacy restoration, public registry compatibility, runtime loading,
and authored Wasteland/Overgrown truth. All capabilities remain strict server-
owned opt-ins and default false. Current authoritative simulation remains 20Hz.

Batch 35 is complete and pushed on main as `feat(maps): author first Reforged
arenas`. Wasteland Outpost and Overgrown Suburb now have strict-validated 40x24
successors behind literal server-owned `largeWorlds`; all authoritative launch
paths resolve the variant, while the client consumes only the normalized
handshake choice. Both layouts cover routes/flanks, landmarks, three KOTH
anchors, centered Core Run, four quadrant spawns, pickup economy, destructible
cover, shootable gates, explosive barrels, minimap metadata, connectivity, and
asymmetry review. All six legacy JSON bytes and the public registry remain
unchanged. Strict/stable-order tooling, 7 focused files/166 tests, the
142-file/1,623-test full unit matrix, static checks, affected/full builds, the
145-pass default-false browser matrix, and the final 166-pass enabled three-
project matrix passed. Repository-wide Prettier still reports 100 unrelated
historical files; the Batch 35 allowlist is formatted. Batch 35 skipped
deployment; production remains on the separately user-approved Batch 33 commit
`f39eb34131f8827f85432aafcc6d6c18a2d0ac51`, healthy at 20Hz with every
capability default false.

Choose and document the Batch 36 authored-layout/runtime-loading verification
tier. Run the strict validator and stable-order CLI on all four successors;
...
formatting; `git diff --check`; intended-diff and legacy-byte review. Because
the successor documents become runtime-loadable behind `largeWorlds`, run the
complete three-project browser matrix plus targeted desktop Chromium and
mobile-landscape visual evidence for both new 40x24 arenas, camera edges, spawn
quadrants, routes/landmarks, destruction/gates/hazards, objectives, HUD/minimap,
capability-off/old-server legacy restoration, Results/rematch, and recovery.
Under RFG-003, use Chromium for live/compositor pixels and staged Firefox/mobile
object/input plus direct-renderer evidence.
...
## Batch 37 input prompt (historical)
...
docs/REFORGED_MAP_AUTHORING.md, docs/REFORGED_STYLE_BIBLE.md,
docs/REFORGED_ASSET_PIPELINE.md, and
docs/reforged/style-bible/PROVENANCE.md before implementation. Batch 37 -
Checkpoint Zero and Rusted Refinery is complete. Implement Batch 38 - Mode and
bot rebalance exactly as specified and do not begin Batch 39 - Reforged Arena
release gate.

Validate and evidence all eight standard modes - Deathmatch, KOTH, Gun Game,
Last Stand, Kill Confirmed, One in the Chamber, Core Run, and Bounty Hunt -
across the complete six-arena `standard-40x24` set. Cover legal Duel, Rumble,
and Crew compositions; four-quadrant spawn safety; objective and contest travel
times; pickup density and reachability; shootable-gate, destructible-cover, and
existing explosive-barrel interactions; bot path reachability, target choice,
objective participation, pickup use, unsticking, and recovery; and regulation
pacing through deterministic server-owned evidence. Tune only shared declarative
mode, spawn, pickup, navigation, or bot parameters where measured evidence
proves a cross-arena problem. Keep base movement and stamina unchanged unless
the evidence requires a separate future tuning batch. Do not solve presentation
concerns by changing mechanics.

Batch 38 owns only evidence-driven cross-arena mode/bot rebalance. Preserve all
six hand-authored successor documents byte-for-byte unless a proven shared
contract correction is required and documented. Preserve all six byte-identical
20x12 legacy JSON files, public map names, public `MAP_REGISTRY`, wire payloads,
literal server-owned variant selection, and capability-off/old-server legacy
restoration. The authoritative server and capability handshake must continue
choosing the variant; the client may consume that choice but must not infer it
from viewport size, map name, local configuration, or art availability. No
40x24 arena may enter default matchmaking or production.

Do not begin the Batch 39 release gate, expose capabilities, change defaults,
deploy, or smoke production. Do not add tactical-map gameplay, Battle Royale
systems, rarity/loot/containers, new hazard types, spectating, new modes, or a
second client-owned map-selection surface. Batch 39 owns the complete Reforged
Arena release review, blocker disposition, any capability exposure, and any
separately user-approved production deployment.
...
and recovery behavior byte-for-byte. Preserve Batch 34 stable validation codes,
strict/compatible loading profiles, fail-closed result, stable-order tooling,
and all old-schema fixtures. Preserve Batch 35-37 literal server-owned variant
selection, legacy restoration, public registry compatibility, runtime loading,
and all six authored successors. All capabilities remain strict server-owned
opt-ins and default false. Current authoritative simulation remains 20Hz.

Batch 37 is complete and pushed on main as `feat(maps): author final Reforged
arenas`. Checkpoint Zero and Rusted Refinery complete the six strict-validated
40x24 successors behind literal server-owned `largeWorlds`; all authoritative
launch paths resolve the variant, while the client consumes only the normalized
handshake choice. Both new layouts cover routes/flanks, landmarks, three KOTH
anchors, centered Core Run, four quadrant spawns, pickup economy, destructible
cover, shootable gates, explosive barrels, minimap metadata, connectivity, and
asymmetry review. All six legacy JSON bytes, all four Batch 35-36 successor
bytes, and the public registry remain unchanged. Strict/stable-order tooling,
7 focused files/111 tests, the 142-file/1,625-test full unit matrix, static
checks, affected/full builds, the 145-pass default-false browser matrix, and the
166-pass literal-`largeWorlds` three-project matrix passed. Desktop Chromium and
Firefox/mobile direct-renderer evidence was inspected for both new arenas under
RFG-003. Repository-wide Prettier reports 109 unrelated historical/environment
files; the Batch 37 allowlist is formatted. Batch 37 skipped deployment;
production remains on the separately user-approved Batch 33 commit
`f39eb34131f8827f85432aafcc6d6c18a2d0ac51`, healthy at 20Hz with every
capability default false.

Choose and document the Batch 38 cross-arena simulation/balance verification
tier before implementation. Build deterministic coverage that can compare all
eight modes and legal participant/team shapes across all six successors without
relying on wall-clock flakiness. Run focused positive/negative mode, spawn,
objective, pickup, navigation, bot, destruction/gate/hazard, Results/rematch,
recovery, registry/selection, strict/stable validation, legacy/old-schema, and
capability-off tests; the full unit matrix because mode and bot changes cross
shared/server/client authority; typecheck; lint; affected and full production
builds; repository formatting; `git diff --check`; intended-diff and protected-
byte review. Run targeted three-project browser and deterministic server evidence
for all affected journeys, desktop Chromium plus mobile-landscape interaction/
visual checks, and the relevant 20Hz tick-budget/performance probes. Escalate to
the complete browser matrix if focused evidence or the actual change surface
crosses a foundation, and document that decision. Do not run Batch 39's release
or deployment gate early.

Update roadmap acceptance/status, architecture/baseline/capability and map-
authoring docs where the proven balance contract changes, bug ledger only with
proven evidence, and Session Log. Run the complete end-of-batch ritual, commit
and push directly to main, verify a clean worktree with HEAD exactly matching
origin/main, and skip deployment.
