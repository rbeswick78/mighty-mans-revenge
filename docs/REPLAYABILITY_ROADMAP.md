...
**Shipped:** map and mode drafts now work from every input surface. Tab moves
linearly through enabled cards, arrows preserve the two-column spatial layout,
and Enter confirms the focused choice. Escape and Backspace still return to
the lobby. When an authoritative snapshot advances from the map phase to the
mode phase, keyboard and gamepad focus hands off to the first enabled card
instead of disappearing or remaining on an invalid choice.

Pointer input clears stale keyboard/gamepad focus before a mouse click or tap
takes over. Desktop guidance now explains Tab/arrows/Enter and the lobby exit;
touch guidance explains the direct card vote and Rally tie rule. Duel picks,
Rally ballots, server authority, matchmaking, combat, balance, persistence,
and networking behavior are unchanged.

**Verification:** typecheck and lint pass; all 1,331 unit and integration tests
pass across 105 files. The production build passes with the existing Vite
chunk-size advisory (`index-DPyOCQt5.js`, 1,869.19 kB / 447.20 kB gzip).
The dedicated Draft regression passes in Chromium, Firefox, and 844×390 mobile
landscape, proving linear and spatial keyboard navigation, cross-column focus,
Enter activation, authoritative phase handoff, pointer takeover, real touch
selection, and adaptive guidance. The complete 141-case Playwright matrix
validates all 126 executable cases with 15 intentional project-specific skips.

**Operational watch:** future Draft phases must continue to expose one ordered
enabled-card list so keyboard and gamepad focus hand off together when server
snapshots replace the available choices.
...
**Shipped:** every lobby route is now reachable from a desktop keyboard. Tab
leaves the callsign field and enters the action list, arrows and subsequent Tab
presses move through all ten online/local actions with the existing visible
button focus, and Enter activates the focused route. Escape closes Practice
Setup or cancels an active search first, then returns editing ownership to the
callsign field. Before navigation begins, Enter retains its fast Quick Match
shortcut and left/right retain ordinary callsign caret behavior.

Pointer input now clears stale keyboard/gamepad focus before taking over, so a
mouse or tap never leaves two routes highlighted. The footer explains the
active device instead of advertising gamepad controls to everyone: desktop
shows the keyboard/gamepad contract, while touch displays a short tap and
landscape cue. Firefox's duplicate bubbling Tab delivery is ignored through
the event's consumed state, keeping one physical press equal to one move.
Gamepad order, touch targets, route callbacks, matchmaking, Practice settings,
combat, balance, persistence, networking, and server behavior are unchanged.

**Verification:** typecheck and lint pass; all 1,331 unit and integration tests
pass across 105 files. The production build passes with the existing Vite
chunk-size advisory (`index-BKVsjUNq.js`, 1,865.77 kB / 446.61 kB gzip).
Nine affected lobby journeys pass across Chromium, Firefox, and 844×390 mobile
landscape. The dedicated regression enumerates every desktop route, proves
callsign caret isolation, local Enter activation, nested Escape behavior,
focus wrap, real pointer takeover, adaptive guidance, and real mobile taps.
The complete 135-case Playwright matrix validates all 120 executable cases
with 15 intentional project-specific skips.

**Operational watch:** any future lobby action belongs in the shared ordered
button list so keyboard and gamepad navigation inherit it together. Keep
callsign editing isolated until Tab deliberately hands control to that list.

### Session 106 — 2026-07-15 — Readable Lobby Play Choices

**Shipped:** the lobby's dense play menu now makes every route readable before
the player commits. Spar, Scrap Pit, and Crew Battle use larger route titles
with their rival or local-record context separated into a quieter second line;
Gauntlet and Daily Run retain one-line titles with matching visual weight.
Practice Setup and the Build Codex also gain larger labels and clearer spacing.
The primary Quick Match and Rumble controls keep their established hierarchy.

Visible buttons and their invisible pointer zones are taller, with every
adjacent row kept non-overlapping. This preserves forgiving touch input after
the fixed 960×720 canvas scales into an 844×390 mobile landscape viewport.
All route callbacks, local progression, matchmaking requests, Practice setup,
network messages, combat, balance, and server behavior are unchanged.

**Verification:** typecheck and lint pass; all 1,331 unit and integration tests
pass across 105 files. The production build passes with the existing Vite
chunk-size advisory. A dedicated geometry guard verifies exact route titles,
subtitles, text containment, font hierarchy, button bounds, expanded pointer
targets, and non-overlap on Chromium, Firefox, and 844×390 mobile landscape.
The five affected browser journeys pass with 21 executable cases and three
intentional project-specific skips. The complete Playwright matrix was also
run before release.

**Operational watch:** keep future lobby routes inside this two-tier hierarchy.
Put the durable action name on the button title and progression or opponent
context in the subtitle instead of shrinking both into one line.
...
**Shipped:** live fights now explain every elimination in the HUD strip. The
existing five-row feed was fully rendered but never connected to the reliable
`server:playerKilled` event, leaving victims with no visible answer to who
eliminated them or how. GameScene now projects that authoritative event into a
compact line such as `YOU [RIFLE] RUSTY` or `RUSTY [SHOTGUN] YOU` and keeps the
newest outcome at the top. Local kills use mint, local deaths use the brighter
combat-danger red, and the rest of the field stays bone-white.

All nine authoritative kill sources receive explicit player-facing names;
long callsigns are bounded and a departed/missing roster entry falls back to
`FIGHTER`. Self-eliminations read `SELF` instead of inventing another player.
The feed remains capped at five rows inside the existing right HUD column and
expires on its established timer. This is presentation only: attribution,
combat, scoring, kill feed events, audio, persistence, and server behavior are
unchanged.

**Verification:** typecheck and lint pass; all 1,327 unit and integration tests
pass across 104 files. The full production build passes with the existing Vite
chunk-size advisory (`index-DG0Yub9f.js`, 1,862.78 kB / 445.96 kB gzip). Pure
coverage checks local kill/death emphasis, neutral/missing names, suicides, and
all nine kill sources. The focused real-event browser journey passes on
Chromium, Firefox, and 844×390 mobile landscape, verifies newest-first order,
exact colors, 16px outlined type, and full HUD bounds. The complete 123-case
Playwright matrix passes with 108 executable passes and 15 intentional
project-specific skips in 14.1 minutes.

**Operational watch:** keep future kill-source names compact enough for the
right HUD column and add them to the exhaustive typed presentation map. The
reliable server event remains the only attribution source; never infer a kill
from trails, local health transitions, or rendered corpses.
...
**Shipped:** Character Select no longer asks players to decode six miniature
ability paragraphs at once. The roster cards are shorter and keep the identity
scan to portrait, name, mastery, health, and speed. A dedicated full-width
briefing now follows the active fighter with a readable ability name, exact HP
and speed, and one plain-language rule/cooldown line. Pointer and touch update
the briefing on the first selection; keyboard and gamepad cycling update it
immediately; authoritative server hover/lock state remains the final source of
truth.

The briefing copy lives in a small typed presentation module backed by the
shared character definitions, so health and speed cannot drift from gameplay
configuration. Its six concise ability rules are deliberately decision-oriented
(`SHOOT THROUGH WALLS`, `HALF DAMAGE`, `DASH 3 TILES`) rather than repeating
flavor text players cannot comfortably read before the auto-lock timer expires.

**Verification:** typecheck and lint pass; all 1,312 unit and integration tests
pass across 99 files. Unit coverage checks every fighter's exact presentation
and keeps detail copy bounded. The production build passes with the existing
Vite chunk-size advisory (`index-X2mEW897.js`, 1,858.90 kB / 444.75 kB gzip).
The focused Chromium and 844×390 mobile journeys physically click/tap Jack,
cycle to Rook by keyboard, verify readable 11/13px typography, prove the old
miniature cooldown copy is gone, and keep the lock action clear of the footer.
The complete 108-case Playwright matrix passes cleanly with 93 executable
passes and 15 intentional cross-project skips, including this briefing journey
on Chromium, Firefox, and mobile landscape.

**Operational watch:** ability rules are intentionally short enough for the
fixed 960×720 canvas and phone-scale rendering. Keep future fighter copy within
the same one-line budget; if a power genuinely needs more explanation, teach
its edge case in live HUD feedback instead of shrinking this decision surface.
...
**Shipped:** live combat now has a persistent top-right `MENU` action for
pointer/touch, keyboard Escape, and standard gamepad Start. The non-pausing
overlay supports D-pad/A/B navigation, says clearly that combat continues, and
requires a second confirmation before leaving. Its consequence copy is
specific to a duel forfeit, Rumble elimination, ordinary Practice, Crew group,
or Gauntlet/Daily run. While open, touch gameplay is disabled and outgoing
movement/action input is neutralized so navigating the overlay cannot buffer a
shot, ability, grenade, or movement command.

Explicit leaves and network disconnects now share one authoritative server
contract. Pre-fight and active Practice groups dissolve cleanly, an active
Rumble eliminates only its leaver, and an active duel awards the remaining
fighter a forfeit win. The leaver's mapping clears immediately, while final
results and post-match consensus include connected fighters only. That lets a
player requeue immediately without a late result or rematch timeout from the
old match reclaiming their new state. The client likewise ignores late
match-end/disconnect races once its confirmed exit begins.

**Verification:** pure copy and gamepad tests cover mode-specific consequences
and edge-triggered Start input. Matchmaking coverage proves active duel
forfeits, explicit Rumble elimination, Character Select disconnect cleanup,
active Practice cleanup, connected-only results, and immediate requeue. The
focused Playwright journey passes on Chromium, Firefox, and 844×390 mobile
landscape using physical pointer/touch input plus keyboard and synthetic
gamepad navigation. Typecheck and lint pass; all 1,310 unit and integration
tests pass across 98 files. The production build passes with the existing Vite
chunk-size advisory (`index-Blw1tnqR.js`, 1,853.98 kB / 443.47 kB gzip).
The 102-case Playwright matrix completed with 85 passes and 15 intentional
cross-project skips; two unrelated late-suite resource timeouts hit the
three-client Chromium Rumble rally and synthetic mobile Crew result. Clean
file/project reruns then passed both Chromium rally cases and both executable
mobile Crew cases, while the live-match menu itself passed on all three
projects in both focused and full-matrix runs. Across the clean results, all 87
executable cases pass.

**Operational watch:** this menu intentionally does not pause an online match,
and the confirmation defaults to `KEEP FIGHTING`. Watch real-phone reach around
the top-right launcher and whether groups want a separate settings surface;
neither should weaken the two-step leave or authoritative forfeit contract.

### Session 94 — 2026-07-14 — Safe Pre-Fight Exits

**Shipped:** Draft and Character Select now provide a consistent visible
`BACK TO LOBBY` control. Pointer and touch use a 150×30 button with a 50px-high
interaction target; Escape, Backspace, and standard gamepad B reach the same
path. Character Select's footer also advertises the keyboard/controller exit
instead of leaving players to discover it by accident.

The client tells the server it is returning before transitioning locally. A
Character Select departure now dissolves the full pre-fight group, clears every
participant's match mapping and associated bot, Practice, rivalry, Rumble, and
rematch state, and notifies the remaining entrants. Both players can therefore
queue again immediately instead of waiting in a stale match. Draft teardown
continues to use its existing equivalent group cleanup, while live-match
return/forfeit behavior remains unchanged.

**Verification:** focused server coverage proves that both Character Select
entrants are released and can form a fresh draft after one leaves. The focused
Playwright journey passes on Chromium, Firefox, and 844×390 mobile landscape,
covering physical pointer/touch input, keyboard Escape, gamepad B, destination
scene, and the expanded hit target. Typecheck and lint pass; all 1,304 unit and
integration tests pass across 97 files. The production build passes with the
existing Vite chunk-size advisory (`index-BCYrguw2.js`, 1,847.26 kB / 441.65
kB gzip). Desktop and mobile captures keep the secondary action clear of the
draft title, cards, fighter roster, and fixed 4:3 safe area. Chromium and
mobile completed cleanly in the 99-case matrix. Two existing Scrap Pit checks
timed out late in the first Firefox leg under resource pressure; the complete
Firefox project then passed cleanly (27 passes, 6 intentional skips). Across
the clean project results, all 84 executable cases pass with 15 intentional
cross-project skips.

**Operational watch:** the button is intentionally secondary and exists only
before combat. A live-match exit should remain a separate confirmed-forfeit
design so a stray touch or controller press cannot concede a fight.

### Session 93 — 2026-07-14 — Lobby Audio Control

**Shipped:** the lobby now exposes the game's existing persistent mute state as
a compact `AUDIO ON/OFF · F2` control. Pointer, touch, keyboard F2, and gamepad
focus all reach the same toggle, including while the lobby is linking or
searching for a match. The control stays deliberately secondary in the
top-right corner so the play choices and callsign flow keep their hierarchy.

The AudioManager now immediately applies the restored `mmr_audio_muted` value
to Phaser's SoundManager as it starts. A reload therefore restores both the
visible label and the actual sound state instead of only remembering the
preference internally. Unmuting provides the usual menu confirmation; muting
does not try to play an inaudible cue. Matchmaking, networking, gameplay,
balance, persistence formats, and fixed canvas dimensions are unchanged.

**Verification:** the focused label test covers both display states. Typecheck
and lint pass; all 1,303 unit and integration tests pass across 97 files. The
production build passes with the existing Vite chunk-size advisory
(`index-vRsnHw11.js`, 1,846.26 kB / 441.51 kB gzip). The focused audio journey
passes on Chromium, Firefox, and 844×390 mobile landscape, physically clicking
or tapping the canvas control, reloading to prove persisted mute, and pressing
F2 to restore sound. The complete Playwright matrix passes 81 tests with 15
intentional project skips across 96 configured cases. Desktop and mobile
captures show a readable secondary control clear of the title, play panel, and
mobile action lanes.

**Operational watch:** F2 avoids typing conflicts in the editable callsign
field and gives keyboard players a direct shortcut. Watch whether players ask
for separate music/effects volume steps later; the intentionally simple mute
control solves the immediate discoverability and persistence gap without
turning the lobby into a settings screen.
