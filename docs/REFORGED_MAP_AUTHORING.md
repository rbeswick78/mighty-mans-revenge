...
established shared contracts. Batch 45 adds the separate private
`battle-royale-56x34` profile described below without widening that standard
registry.
...
| `battle-royale-56x34` | Requires exactly 56x34 tiles at 48px and the complete version-1 private Battle Royale block.                 |
...
## Batch 45 private Battle Royale arena

`shatterlands.battle-royale-56x34.json` is generated by
`corepack pnpm maps:generate:battle-royale`. The strict profile requires exact
56x34 dimensions at 48px, a complete four-region partition, readable transition
footprints, a connected walkable component and connected named region-route
graph, exactly eight two-candidate spawn groups, declared path separation and
safe egress, exact spawn/minimap/landmark inventories, 16 unique reachable
one-cell solid containers, and a complete reachable sustain-only pickup
inventory. Validation is deterministic and does not consume gameplay RNG.

The four named regions are Dust Basin (`wasteland`), Greenward (`overgrown`),
Ironworks (`industrial`), and Glass Wastes (`irradiated`). Their metadata is
runtime projection truth for per-cell presentation and the non-interactive
minimap, while the established tile grid remains collision/navigation truth.
Battle Royale match construction chooses one candidate from each authored
spawn group and registers all container tiles through the existing server loot
boundary. Standard spawn selection and map contracts do not inspect this
metadata.

The arena is not a seventh public map. It is absent from `MAP_REGISTRY`, map
lists, schedules, Draft, Practice, Arena Mastery, standard rematches, standard
persistence normalization, and the 624-product balance matrix. General lookup
may resolve its server-authored name only so the client can load the exact map
named by a Battle Royale match. `getBattleRoyaleMap()` is the authoritative
server selection source and ignores both `largeWorlds` and `FORCE_MAP`.

The minimap projects regions, labels, authored landmarks, mutable collision,
explicit intact-container identities, and permitted local/allied player truth.
An authoritative destroyed-tile refresh removes an opened container. It stays
non-interactive and reveals no generic rival, safe zone, or tactical-map state.
Batch 46 owns those zone and tactical-map additions.

## Scope boundary

Batch 34 owns the standard schema, validator, fixtures, and CLI. Batch 35 owns the first
...
release gate and any production exposure. Batch 45 owns only the private
Battle Royale schema, generator, arena, grouped spawn consumption, authored
container registration, per-cell biome projection, and non-interactive map UI.
Safe zones, tactical-map gameplay, bot behavior, spectating, records,
hardening, capability exposure, and production deployment remain outside this
contract.
