...
- ~~**Wire-fence destructible gate.**~~ **DONE 2026-07-12.** Every arena now carries rotationally paired one-cell `shootable_gate` wall decorations (two per arena; four on Collapsed Overpass). Bullet/pellet impacts, exposed blasts, and Bruce's fire breath permanently open them through the existing reliable tile-destruction path; the client plays the seven-frame strip backward and leaves the open posts visible. No parallel tile type or network state was needed.
...
- ~~**Death animations.**~~ **DONE 2026-07-11.** Registry-driven horizontal death sheets now cover the full five-fighter roster plus Jack's no-axe body. The first-death animation holds its final corpse frame until the authoritative respawn; the unused red-flash/disappear path is gone. Alternate death variations remain available for a future cosmetic-randomization pass.
- ~~**Muzzle flash sprites.**~~ **DONE before Session 21.** `Character/Guns/Fire/Fire_*-Sheet3.png` already drives the directional three-frame flash in `EffectsRenderer.showMuzzleFlash`; the stale backlog note predated the graphics overhaul.
- ~~**Player-hit splashes.**~~ **DONE 2026-07-11.** The server now stamps `BulletTrail` only after authoritative damage application. Confirmed hits replace generic endpoint sparks with one of the pack's two `Enemies/Shot` splashes at bullet-arrival time, while the local shooter hears one deterministic confirmation tick (shotgun pellets collapse to one sound per blast).
