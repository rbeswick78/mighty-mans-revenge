...
test('fighter art I preserves directional states, identity fallbacks, death, and respawn', async ({
  ...
  test.skip(
    ...
    'Run the Batch 28 visual tier in desktop Chromium and mobile landscape.',
  );
  await stageGameplay(page, true, true);
  ...
  const evidence = await page.evaluate(() => {
    const scene = (window as unknown as { game?: Phaser.Game }).game?.scene.getScene(
      ...
      scene: { pause(): void };
      playerManager: {
        updatePlayers(players: unknown[], localPlayerId: string): unknown;
      };
    ...
    const fighter = (
      ...
      velocity: { x: 0, y: 0 },
      aimAngle,
      health: characterId === 'frost_wizard' ? 85 : 100,
      maxHealth: characterId === 'frost_wizard' ? 85 : 100,
      armor: 0,
      ammo: 30,
      weaponId,
      specialAmmo: 0,
    ...
    const roster = [
      fighter('mighty', 'mighty_man', 360, 0, 'rifle'),
      fighter('bruce', 'bruce', 480, -Math.PI / 2, 'rifle'),
      fighter('frost', 'frost_wizard', 600, Math.PI / 2, 'rifle'),
    ];
    scene.playerManager.updatePlayers(roster, 'mighty');
    scene.playerManager.updatePlayers(roster, 'mighty');
    const initial = scene.getReforgedFighterRenderState();

    const ability = roster.map((state) => ({
      ...state,
      abilityActiveSeconds: state.characterId === 'frost_wizard' ? 0 : 1,
      abilityCooldownSeconds: state.characterId === 'frost_wizard' ? 4 : 0,
    }));
    scene.playerManager.updatePlayers(ability, 'mighty');
    const abilityState = scene.getReforgedFighterRenderState();

    const damaged = ability.map((state) => ({ ...state, health: state.health - 10 }));
    scene.playerManager.updatePlayers(damaged, 'mighty');
    const damageState = scene.getReforgedFighterRenderState();

    const dead = damaged.map((state) => ({ ...state, isDead: true, deaths: 2 }));
    scene.playerManager.updatePlayers(dead, 'mighty');
    const deathState = scene.getReforgedFighterRenderState();

    const respawned = roster.map((state) => ({ ...state, deaths: 2, invulnerableTimer: 1 }));
    scene.playerManager.updatePlayers(respawned, 'mighty');
    scene.playerManager.updatePlayers(respawned, 'mighty');
    ...
    const incompatible = respawned.map((state) => ({
      ...state,
      weaponId:
        state.characterId === 'mighty_man'
          ? 'pistol'
          : state.characterId === 'frost_wizard'
            ? 'bat'
            : state.weaponId,
    }));
    scene.playerManager.updatePlayers(incompatible, 'mighty');
  ...
  expect(evidence.initial.map(({ art }) => art.animationKey)).toEqual([
    'reforged-bruce-up-idle',
    'reforged-frost_wizard-down-idle',
    'reforged-mighty_man-side-idle',
  ]);
  expect(evidence.abilityState.every(({ art }) => art.animationKey?.includes('-ability'))).toBe(
    true,
  );
  expect(evidence.damageState.every(({ art }) => art.animationKey?.includes('-damage'))).toBe(true);
  expect(evidence.deathState.every(({ art }) => art.dead)).toBe(true);
  expect(evidence.deathState.map(({ art }) => art.animationKey)).toEqual([
    'reforged-bruce-side-death-v2',
    'reforged-frost_wizard-side-death-v2',
    'reforged-mighty_man-side-death-v2',
  ]);
  expect(evidence.respawnState.every(({ art }) => !art.dead && art.usingModernBody)).toBe(true);
  expect(evidence.fallbackState.map(({ art }) => art.usingModernBody)).toEqual([
    true,
    false,
    false,
  ]);
  ...
  await page.screenshot({ path: testInfo.outputPath('batch-28-fighter-art-i-gameplay.png') });
  if (testInfo.project.name === 'desktop-chromium') {
    await page.setViewportSize({ width: 844, height: 390 });
    await page.evaluate(
      () =>
    ...
    await page.screenshot({
      path: testInfo.outputPath('batch-28-fighter-art-i-mobile-size.png'),
    });
  }
  ...
  writeFileSync(testInfo.outputPath('batch-28-fighter-art-i-direct-renderer.png'), directImage);
  await testInfo.attach('batch-28-fighter-art-i-direct-renderer', {
    body: directImage,
    contentType: 'image/png',
  });
});

test('fighter art I remains legacy when modernArt proof is absent', async ({ page }, testInfo) => {
  test.skip(
    !largeWorldsAdvertised || testInfo.project.name !== 'desktop-chromium',
    'Run the Batch 28 capability fallback proof in desktop Chromium.',
  );
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        ...
        return scene.getReforgedFighterRenderState()[0]?.art ?? null;
      }),
    )
