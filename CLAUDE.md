...
**Crew Battle 2v2 (Session 88):** `client:startPractice.kind = 'crew_battle'` launches an immediate four-fighter Deathmatch with immutable server-authored `blue`/`red` assignments: the human and Rusty versus Scrapjaw and Clank. `Match` owns teammate relationships, combined team score, the first-to-15 target, overtime winner, and optional `winnerTeamId` / `playerTeams` / `teamScores` result fields. Friendly fire is disabled at both target acquisition and the centralized damage choke: hitscan, melee fans, shotgun pellets, axes, grenades, barrels, fire breath, and delayed explosives must skip teammates while self-damage stays legal. `BotController` may only select opponents. The client renders the server map as an `ALLY` marker, a two-score HUD, and grouped roster-authentic Results; it never infers teams from callsign or entry order. Direct Practice rematches retain the same teams, Deathmatch pin, bot identities, difficulty, and compatible Solo Chaos while maps rotate. Crew Battle is Practice: no lifetime stats, rivalry, mastery, leaderboard, Crown, or Grudge writes. Keep team support N-player-safe and do not generalize another mode until that mode has explicit team scoring/objective semantics.
