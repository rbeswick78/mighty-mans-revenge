...
**Wasteland Gauntlet (Sessions 49–53):** `client:startPractice.kind` distinguishes ordinary `sparring` from an optional three-stage `gauntlet`. The server—not the client—assigns Rookie, Scrapper, then Warlord from `PRACTICE_GAUNTLET`, attaches the live stage to `server:matchFound`, resolves advancement from the authoritative winner, and carries only the computed next stage through the normal direct-Practice rematch path. A loss or draw ends the run and resets the next fight to stage 1; a stage-3 win clears it. Advanced results offer server-authored Route A/B map, mode, and Rusty-fighter previews. The selected rival is pinned into the next match, and the server carries an encounter history so no fighter repeats within a run; missing or invalid route input retains Route A compatibility. Gauntlet skips Rivalry Set creation as well as every Practice stats/leaderboard write. Maps, modes, fresh-mutator exclusions, contracts, bot inputs, and combat all keep their ordinary authoritative paths. Keep all Gauntlet and rival metadata optional for old/ordinary Practice payloads, and never let a client-supplied difficulty or rival override a Gauntlet stage.
