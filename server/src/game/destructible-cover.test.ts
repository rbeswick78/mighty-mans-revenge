...
describe('findBlastableCoverTiles', () => {
  it('breaks the first exposed cover but not cover shielded behind it', () => {
    ...
    expect(
      findBlastableCoverTiles(map, createCollisionGrid(map), { x: 120, y: 120 }),
    ).toEqual([{ col: 3, row: 2 }]);
  });
  ...
  it('does not expand from plain cover into an unrelated decoration', () => {
    ...
    expect(
      findBlastableCoverTiles(map, createCollisionGrid(map), { x: 72, y: 120 }),
    ).toEqual([{ col: 2, row: 2 }]);
  });
  ...
  it('never destroys a perimeter wall even when a decoration overlaps it', () => {
    ...
    expect(
      findBlastableCoverTiles(map, createCollisionGrid(map), { x: 72, y: 72 }),
    ).toEqual([]);
  });
